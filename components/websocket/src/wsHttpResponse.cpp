...
WsHttpResponse::WsHttpResponse(std::vector<char> &recvVec,
    ...
    : _state(eParseState::Receving), _recvVec(recvVec), _sendVec(sendVec),
      _remoteIp(remoteIp), _headerDic(), _lastHeader(),
      _lastParseIt(_recvVec.begin()), _httpBodyLen(0),
      _httpResult(eCodes::HTTP_Ok), _config(cfg) {
}
...
std::string WsHttpResponse::createHandshakeSHA1Key() {
    ...
    eCodes code =
        sha1Message((uchar *)swKey.c_str(), swKey.size(), &sha1Buf[0]);
...
eCodes WsHttpResponse::work() {
    ...
    default: { PARROT_ASSERT(false); } break;
    }
