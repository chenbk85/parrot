...
WsParser::WsParser(CallbackFunc cb, std::vector<char> &recvVec,
                   ...
                   const WsConfig &cfg)
    : _recvVec(recvVec), _remoteIp(remoteIp), _needMask(needMask),
      _state(eParseState::Begin), _fin(false), _masked(false),
      _opCode(eOpCode::Continue), _fragmentDataType(eOpCode::Binary),
      _lastParseIt(), _pktBeginIt(), _callbackFunc(cb), _headerLen(0),
      _payloadLen(0), _maskingKey(), _parseResult(eCodes::ST_Ok), _packetVec(),
      _config(cfg) {
    _recvVec.clear();
