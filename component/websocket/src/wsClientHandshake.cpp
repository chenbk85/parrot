...
int WsClientHandshake::onStatus(::http_parser*, const char* at, size_t len)
{
    _headerDic["status"] = std::string(at, len);
    return 0;
}
...
eCodes WsClientHandshake::parse()
{
    ...
    settings.on_status = std::bind(&WsClientHandshake::onUrl, this, _1, _2, _3);
...
std::string WsClientHandshake::createHandshakeSHA1Key()
{
...
void WsClientHandshake::createHttpHandshakeRsp()
{
